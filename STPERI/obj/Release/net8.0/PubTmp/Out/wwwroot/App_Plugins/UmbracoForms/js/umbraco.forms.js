...
angular.module("umbraco").controller("UmbracoForms.SettingTypes.DocumentMapperController", function ($scope, $routeParams, pickerResource) {
  ...
  pickerResource.getAllDocumentTypes().then(function (response) {
    ...
    $scope.$watch(
      ...
      function () {
        if (typeof vm.htmlfield !== 'undefined') {
          $scope.setting.value = vm.htmlfield.value.markup;
        }
      }, true);
...
angular.module("umbraco").controller("UmbracoForms.SettingTypes.TextWithFieldPickerController",
  function ($scope) {
    const vm = this;
    vm.model = $scope.model;
    vm.setting = $scope.setting;
    vm.fieldDdlValue = '';
    vm.textBoxValue = '';
    vm.inputMode = '';
    vm.fieldDdlOptions = [{ caption: '--- Choose ---', alias: '' }, ...vm.model.fields];

    /**
     * Called by angular when component is initializing.
     */
    vm.$onInit = () => {
      vm.setting.value = vm.setting.value || vm.setting.defaultValue || '';

      if (isFieldMagicString(vm.setting.value)) {
        vm.inputMode = 'FIELD_PICKER';
        vm.fieldDdlValue = deMagic(vm.setting.value);
      }
      else {
        vm.inputMode = 'FREE_TEXT';
      }

      vm.textBoxValue = vm.setting.value;
    }

    vm.changeInputMode = (inputMode) => {
      vm.inputMode = inputMode;
    }

    vm.onFieldDdlChange = () => {
      const magicString = toMagicString(vm.fieldDdlValue);
      vm.setting.value = magicString;
      vm.textBoxValue = magicString;
    }

    vm.onTextBoxChange = () => {
      vm.setting.value = vm.textBoxValue;
      if (isFieldMagicString(vm.textBoxValue)) {
        vm.fieldDdlValue = deMagic(vm.textBoxValue);
      }
      else {
        vm.fieldDdlValue = '';
      }
     ...
     */
    function toMagicString(input) {
      if (!input) {
        return '';
      };

      if (isMagicString(input)) {
        return input;
      }
     ...
     */
    function deMagic(input) {
      return isMagicString(input) ? input.substring(1, vm.setting.value.length - 1) : input;
    }

    function isMagicString(input) {
     ...
     * @param {string} input 
     */
    function isFieldMagicString(input) {
      if (!isMagicString(input)) {
        return false;
      }

      const fieldAliases = vm.model.fields.map(field => field.alias);
      return fieldAliases.includes(deMagic(input));
    }
  });
...
angular.module("umbraco").controller("UmbracoForms.Editors.Form.DeleteController", function ($scope, $q, formResource, recordResource, navigationService, notificationsService, treeService, localizationService, utilityService) {
  var vm = this;
  ...
  if (!vm.deletingFolder) {
    ...
    // Display warning for records if deleting a form.
    vm.recordDetail = "";
    var filter = {
    ...
    // Display warning for relations if one or more relations exist for the form.
    vm.showWarningForRelations = false;
    promises.push(
      formResource.hasRelations(vm.id)
        .then(function (response) {
          vm.showWarningForRelations = response.data;
        }));
  }
  ...
  function createAndLocalizeApps() {
    localizationService.localizeMany([
      ...
      $scope.page.contentApps.unshift({
        ...
        "alias": "info",
        "icon": "icon-info",
        "view": "/App_Plugins/UmbracoForms/backoffice/Form/views/info/info.html",
        "active": false
      });
  ...
  $scope.save = function () {
    if (formHelper.submitForm({
      ...
      formResource.saveWithWorkflows($scope.form).then(function (response) {
        ...
        if (err.data.ExceptionType.startsWith("System.OperationCanceledException")) {
...
angular.module("umbraco").controller("UmbracoForms.Editors.Form.EntriesController", function ($scope, $routeParams, $location, recordResource, formResource, editorService, userService, securityResource, notificationsService, navigationService, overlayService, localizationService, utilityService, preValueSourceResource, formService) {
  var vm = this;
  ...
  function loadRecords(filter, append, displayEntryId) {
    // Set loading to true
    $scope.loading = true;
    ...
    recordResource.getRecords(filter).then(function (response) {
      ...
      if (displayEntryId) {
        for (var j = 0; j < $scope.records.results.length; j++) {
          if ($scope.records.results[j].uniqueId === displayEntryId) {
            vm.viewEntryDetails($scope.records.schema, $scope.records.results[j]);
            break;
          }
        }
      }
    ...
    securityResource.getForCurrentUser(false).then(function (response) {
      ...
      if (!$scope.hasAccessToCurrentForm || !$scope.security.userSecurity.viewEntries) {
        ...
        formResource.getWithWorkflowsByGuid($routeParams.id).then(function (response) {
          ...
          // See if we have a requested entry Id.  If so, we want to load the page of records that the entry is on.
          // Then load the records. We need to do this after retrieving the form in order to be able to use
          // the selected fields for display that are part of the form's settings.
          var requestedEntryId = $location.search()["id"];
          if (requestedEntryId) {
            $scope.filter.recordId = requestedEntryId;
            recordResource.getPageNumberForRecord($scope.filter).then(function (response) {
              var pageNumber = response.data;
              if (pageNumber) {
              ...
              loadRecords($scope.filter, false, requestedEntryId);
            })
          }
          else {
  ...
  $scope.filter = {
    ...
    states: ["Approved", "Rejected", "Submitted"],
  ...
  vm.viewEntryDetails = function (schema, entry, event) {
    ...
    $location.search("id", entry.uniqueId);
  ...
  vm.fieldValueOrPrevalueCaption = function (value, fieldId) {
    ...
    if (formField.preValues && formField.preValues.length > 0) {
      ...
      for (var i = 0; i < formField.preValues.length; i++) {
        prevalue = formField.preValues[i];
      ...
      if (!matchedWithPrevalue && value.includes(prevalueValueSeparator)) {
        ...
        for (var j = 0; j < valueParts.length; j++) {
          ...
          for (var k = 0; k < formField.preValues.length; k++) {
            prevalue = formField.preValues[k];
  ...
  vm.openFieldTypePicker = function (field) {
    ...
    var fieldTypePicker = {
      ...
      submit: function (model) {
        ...
        field.mandatory = field.$fieldType.supportsMandatory && field.$fieldType.mandatoryByDefault;
        editorService.close();
      },
      close: function (model) {
        editorService.close();
...
angular.module("umbraco").controller("UmbracoForms.Overlays.WorkflowsOverviewOverlayController", function ($scope, workflowResource, notificationsService, editorService, overlayService, localizationService) {
  ...
  if (!$scope.model.formWorkflows.onReject) {
    $scope.model.formWorkflows.onReject = [];
  }
  ...
  vm.workflowsSortableOptions = {
    ...
    stop: function (event, ui) {
      ...
      updateSortOrder($scope.model.formWorkflows.onReject);
    }
  };
...
angular.module("umbraco").controller("umbTrackedFormReferencesController", function ($q, $routeParams, formResource, dateHelper, userService, udiParser) {
    var vm = this;
    ...
    function onInit() {
        ...
        vm.noReferencesTitle = "This item has no references";
        ...
        // If the current url contains create=true, this means that the form is being created, so it has no references yet.
        if (!$routeParams.create) {
            // Make array of promises to load:
            var promises = [loadReferencesRelations(), getByGuid()];
    ...
    function loadReferencesRelations() {
        return formResource.getRelations(vm.id, vm.pageNumber, vm.pageSize)
            .then(function (data) {
                vm.references = data.data;
                vm.hasReferences = vm.references.items.length > 0;
    ...
    function getByGuid(){
        return formResource.getByGuid(vm.id).then(function (data) {
            vm.form = data.data;
            userService.getCurrentUser().then(currentUser => {
                vm.form.createDateFormatted = dateHelper.getLocalDate(vm.form.created, currentUser.locale, 'LLL');
                vm.form.updateDateFormatted = dateHelper.getLocalDate(vm.form.updated, currentUser.locale, 'LLL');
            });
        });
    }
...
(function () {
  ...
  function FormSettingsController($scope, localizationService, overlayService) {
    ...
    function initDataRetention() {
      ...
      vm.showDaysToRetainRejectedRecords = $scope.model.daysToRetainRejectedRecordsFor > 0;
    }
...
angular.module("umbraco").controller("UmbracoForms.Editors.Security.EditController", function ($scope, $routeParams, securityResource, notificationsService, navigationService, localizationService, editorService, usersResource, userGroupsResource) {
  ...
  vm.security = {};
  vm.loading = true;
  vm.allFormsAccess = true;
  vm.formSecuritySortBy = "formName";
  vm.formSecuritySortDirection = "asc";
  ...
  var initializeSecurity = function (data) {
    vm.security = data;
    setAllFormsAccessValue(vm.security);
    vm.loading = false;
  };

  // When form security page is loaded, if a toggle box is set to false, select/deselect all toggle box should also be set to false.
  var setAllFormsAccessValue = function(data){
    data.formsSecurity.every(form => {
      if (!form.hasAccess) {
        vm.allFormsAccess = false;
        return false;
      }
  ...
  vm.toggleAllFormsAccess = function () {
    vm.allFormsAccess = !vm.allFormsAccess;
    vm.security.formsSecurity.forEach(form => {
      form.hasAccess = vm.allFormsAccess;
    });
  }

  vm.toggleFormAccess = function (form) {
    form.hasAccess = !form.hasAccess;
    changeAllFormsAccessValue(form);
  };

  vm.sortFormSecurityBy = function (sortBy) {

    // If switching column, revert to ascending sort. Otherwise switch from whatever was previously selected.
    if (vm.formSecuritySortBy !== sortBy) {
      vm.formSecuritySortDirection = "asc";
    } else {
      vm.formSecuritySortDirection = vm.formSecuritySortDirection === "asc" ? "desc" : "asc";
    }

    // Sort by the new column.
    vm.formSecuritySortBy = sortBy;
    vm.security.formsSecurity = _.sortBy(vm.security.formsSecurity, function (i) { return sortBy === "formName" ? i.formName.toLowerCase() : i[sortBy] });

    // Reverse the order if sorting descending.
    if (vm.formSecuritySortDirection === "desc") {
      vm.security.formsSecurity = vm.security.formsSecurity.reverse();
    }
  };

  vm.isSortDirection = function (sortBy, sortDirection) {
    return vm.formSecuritySortBy == sortBy && vm.formSecuritySortDirection === sortDirection;
  };

  var changeAllFormsAccessValue = function(form){
    var formSecurityList = vm.security.formsSecurity;
    var formSecurityListWithCurrentFormAccessValue = formSecurityList.filter(v => v.hasAccess == form.hasAccess);

    // If user manually set all toggle boxes to the same value, the select/deselectall toggle box should also be set to the same value.
    if (formSecurityListWithCurrentFormAccessValue.length == formSecurityList.length) {
      vm.allFormsAccess = form.hasAccess;
    }
  }
...
angular.module('umbraco.resources').factory('formResource', function ($http, umbRequestHelper, localizationService, notificationsService) {
  ...
  return {
    ...
    getRelations: function (id, pageNumber, pageSize){
      return $http.get(apiRoot + "GetRelations?id=" + id + "&pageNumber=" + pageNumber + "&pageSize=" + pageSize);
    },
    hasRelations: function (id) {
      return $http.get(apiRoot + "HasRelations?id=" + id);
    }
  };
});
...
angular.module('umbraco.resources').factory('recordResource', function ($http) {
  ...
  return {
    ...
    getPageNumberForRecord: function (filter) {
      return $http.post(apiRoot + "PostPageNumberForRecord", filter);
    },
...
angular.module("umbraco.directives").directive('umbFormsDesignerNew', function (formService, fieldResource, workflowResource, notificationsService, editorService, overlayService, localizationService) {
  return {
    ...
    link: function (scope, element, attrs, ctrl) {
      ...
      scope.addPageToTopButton = {
        labelKey: "formEdit_addToTop",
        handler: function () { scope.addPageToTop(scope.form); },
        alias: "addPageToTop",
      };
      scope.addPageSubButtons = [{
        labelKey: "formEdit_addToBottom",
        handler: function () { scope.addPageToBottom(scope.form); },
        alias: "addPageToBottom",
      }];
      ...
      // *********************************************
      scope.addPageToTop = function (form) {
        formService.addPage(form, true);
      };

      scope.addPageToBottom = function (form) {
        formService.addPage(form, false);
      };
...
angular.module('umbraco.services').factory('formService', function (preValueSourceResource) {
  ...
  var service = {
    ...
    addPage: function (form, addOnTop) {
      var p = {
        caption: "",
        fieldSets: [],
        id: generateGuid()
